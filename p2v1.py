### 2번 시나리오 ver 1
# 제출 의도
# naive 한 방법으로, 비어있는 곳을 확인하고 구간을 찾아 붙인다
# 실패하는 경우가 생기지 않고 답변 날짜가 매우 빠르기 때문에 효율성에서 만점이 나온다
# 하지만 패널티 점수가 매우 커서 불리하다.

# 결괏값
# {'accuracy_score': 80.0, 'efficiency_score': 10.0, 'penalty_score': 101.87391353144484, 'score': 488.1260864685552}
...
# 가설 : 한 번도 답변을 안할 경우
def testAPI():
    auth_key = startAPI(BASE_URL, 1)
    for i in range(0, 200):
        newRequestAPI(BASE_URL, auth_key)
        dic = dict()
        dic["replies"] = []
        replyAPI(BASE_URL, auth_key, dic)
        dic = dict()
        dic["room_assign"] = []
        simulateAPI(BASE_URL, auth_key, dic)
    scoreAPI(BASE_URL, auth_key)
...
def solve(problem_num, h, w, day):
    ...
    for cnt in range(1, day + 1):
        ...
        print(roomAssign)
        simulateAPI(BASE_URL, auth_key, roomAssign)
