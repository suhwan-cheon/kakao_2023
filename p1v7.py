### 1번 시나리오 ver 7
# 제출 의도
# 해당 날짜에서 객실 수 * 기간이 가장 높은 것부터 잡는다

# 결괏값
# {'accuracy_score': 80.0, 'efficiency_score': 10.0, 'penalty_score': 179.18098523712447, 'score': 410.81901476287555}
...
def solve(problem_num, h, w, day):
    ...
    for cnt in range(1, day + 1):
        ...
        for request in requests:
            req.append((request["id"], request["amount"], request["check_in_date"], request["check_out_date"], 
            request["amount"] * (request["check_out_date"] - request["check_in_date"]))) 
        req.sort(key = lambda element: element[4])
